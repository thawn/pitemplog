		...
		<div class="item active">
			...
			<div class="container">
				<div class="carousel-caption">
					...
					<p>All available sensors and their IDs should be listed here. Just add the database and Sensor information and the
						data will show up in the graphs.</p>
				</div>
			</div>
		</div>
	</div>
		...
		case 'sensor' :
			if ($value != $sensor) {
				if ($value != substr( $sensor, 3 )) {
					abortWithError( 'Error: sensor prefix and value do not match. Something is wrong:<pre>' . (print_r( [ 
							$sensor => $value
					], true )) . '</pre>' );
				}
			}
...
foreach ( $conf as $sensor => $config ) {
	if ($sensor == 'database') {
		...
		if ((empty( $errors['nameErr'] ) || empty( $errors['nameErr'][$sensor] )) && (empty( $errors['tableErr'] ) || empty( $errors['tableErr'][$sensor] )) && (empty( $errors['calibrationErr'] ) || empty( $errors['calibrationErr'][$sensor] )) && (empty( $errors['categoryErr'] ) || empty( $errors['categoryErr'][$sensor] ))) {
			$hasErrors = false;
		} else {
			$hasErrors = true;
		}
