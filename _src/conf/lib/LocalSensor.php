...
class LocalSensor extends AutoAssignProp {
	...
	function set_sensor(string $val) {
	...
	function set_calibration(string $val) {
	...
	function set_category(string $val) {
		$this->category = $this->filter_default( $val, 'category' );
	}
	function set_confirmed(string $val) {
		$this->confirmed = $val === 'true' ? 'true' : '';
	}
	function set_enabled(string $val) {
		$this->enabled = $val === 'false' ? 'false' : 'true';
	}
	function set_comment(string $val) {
	...
	function set_tabletest(string $val) {
	...
	function update_config(array $data) {
	...
	function update_table($table_old) {
	 ...
	 */
	function reset_aggregate_tables() {
		if ($this->table) {
			$partition_database = escapeshellcmd( '/usr/local/bin/partition_database.py' );
	 ...
	 * @return boolean
	 */
	function table_exists($table_in = NULL) {
		$result = FALSE;
	 ...
	 */
	function merge_db_tables($table_old) {
