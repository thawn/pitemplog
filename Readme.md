...
* Solder the wires of the cable to the thermometer (see [picture below](#sensor-pinout)). In this example, the cable has white green and red wires. For example:
  1. white to pin 1: GND
  * green to pin 2: DQ
  * red to pin 3: V<sub>DD</sub>
* solder the other end of the cable to the 3-pin socket:
...
* solder the wires of another plug parallel to the socket (see [picture below](#assembled-sensor)):
  1. black to pin 1
  * brown to pin 2
  * red to pin 3 
...
* Flash the image files to the sd card for example using [etcher](https://etcher.io/).
* You may want to [enable ssh](https://www.raspberrypi.org/documentation/remote-access/ssh/) by putting a file named ssh on the boot partition. Don't forget to change the password using raspi-config after you boot up the raspi and ssh into it using the default username `pi` and password `raspberry`.
* Insert the sd card into the raspi.
* Connect the usb disk to the raspi.
* Connect a power cable to the raspi. The raspi will boot up and copy some files onto the usb disk. Do not power down the raspi or disconnect the usb disk until the green lights on the raspi and the activity lights on the usb disk stop blinking. **Warning!!! This will overwrite all data on the first partition of the usb disk!!!**
* See [Hardware setup](#hardware) for how to connect the temperature sensors
* See [Configuration](#config) for how to configure the web frontend and start logging data.
* Done!
...
* [Enable ssh](https://www.raspberrypi.org/documentation/remote-access/ssh/) by putting an empty file named `ssh` onto the boot partition of the sd card
* Insert the sd card into the raspberry pi and connect the power cable in order to start the raspi
* Figure out the IP address of the raspi either from your router's web interface or by connecting a monitor to the raspi. It will tell you it's ip address at bootup. With some routers you can also use the hostname `raspberrypi` to connect to the raspi.
* Log into the raspi via ssh: `ssh p@<ip address>`
* add the following lines to `/boot/config.txt`:
```
dtoverlay=w1-gpio,gpiopin=4,pullup=on
max_usb_current=1
```
* Configure at the following settings using `sudo raspi-config`:
...
* Update the debian system: `sudo apt-get update && sudo apt-get dist-upgrade`
* Install required packages: `sudo apt-get install mysql-server php php-mysql php-curl python-mysqldb python-pip grunt npm git`
* In Debian, jekyll depends on xdg-utils which in turn recommends to install the x-server, which in our case is totally unnecessary. Therefore we install jekyll with the *--no-install-recommends* option: `sudo apt-get install jekyll --no-install-recommends`
* Install peewee and PyYAML: `sudo pip install peewee pyyaml`
* Delete the symlink to the default apache configuration: `sudo rm /etc/apache2/sites-enabled/000-default.conf`
* Clone the git repository: `git clone --depth=1 ssh://git@thawn.noip.me:61022/home/git/websites/temperatures`
* Enter the source directory: `cd temperatures`
* Install the required node modules: `npm install`
* Compile and install the temperature logger and web frontend: `grunt install`
* Create a mysql database and a user with full privileges for that database. If you want to create the default database and user, you can use this command: `sudo mysql < _src/_bin/create_database.sql`
* At this point, it is highly recommended that you move the mysql database directory to an external hard disk because frequent writes to the database file will eventually corrupt the SD card (SD cards only last about 100.000 write cycles, which corresponds to about 80 days if you write to the database every minute. I have had several SD cards fail on me because of this).
 1. Attach a usb harddisk (or a usbstick that supports wear leveling) to the raspi
 * Make sure the harddisk has at least one partition and that you don't need any of the data on it.
 * This step will **overwrite the harddisk in /dev/sda1** so make sure that is the **correct one** and that you **don't need the data on there any more**!!! `/usr/local/share/templog/_sbin/setup_usb_storage.sh`
* Reboot the raspi with `sudo reboot`
* See [Hardware setup](#hardware) for how to connect the temperature sensors
* See [Configuration](#config) for how to configure the web frontend and start logging data.
* Done!
...
* Instead of step 9 install only the [minimal dependencies](#deps): `sudo apt-get install mysql-server php php-mysql php-curl python-mysqldb python-pip`.
* Continue to follow the manual installation until step 12.
* On your development machine install the [development dependencies](#devdeps). On a debian machine: `sudo apt-get install python-pip grunt npm git jekyll`.
* Steps 13 - 15 are done on your development machine.
* Deploy the logger and web frontend by running the following on your development machine: `grunt deploy --host=<ip address or hostname of your raspi>`.
* Now continue ssh back into the raspi and continue there: `sudo mysql < /usr/local/share/templog/_bin/create_database.sql`.
* If you want to use an external harddisk (recommended!): `sudo /usr/local/share/templog/_sbin/setup_usb_storage.sh`. This will cause the raspi to format and set up an external harddisk the first time it is available during boot. On subsequent boots, the harddisk is not formatted but must be available otherwise the database will not run (because its data is stored on the harddisk).
* In order to save space, we uninstall pip: `sudo apt-get remove python-pip`.
* Now we clean up unused packages and package files: `sudo apt-get autoremove && sudo apt-get clean`. With Raspbian Stretch Lite (2017-11-29) there was 1.4GiB used on the root partition of my raspi. The gzipped image will be considerably smaller (<500 MiB).
* The following steps can be skipped if you don't want to create an image:
  ...
  * Overwrite free space with zeros to reduce the image file size later:
```
sudo -s
...
rm zero.file
```
  * Disable ssh: `sudo update-rc.d ssh disable`.
  * Shutdown the raspi: `sudo halt`.
  * Connect the sd card to your development machine.
  * Shrink the ext4 partition to ~1900MB using gparted (to allow installation on a 2GB sd card).
  * Identify the sd card with `parted -l`
  * Create an image (important: use the sd card device not the partition. i.e. use /dev/sdc and not /dev/sdc1): `sudo dd if=<sd card device> bs=1m count=2000 | gzip > raspi-templog.img.gz`.
